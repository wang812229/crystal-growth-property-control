...
function values(p:Paper,field:string){const text:Record<string,string>={title:p.title,authors:p.authors,abstract:p.abstract,summary:p.summary,keywords:p.tags.join(' '),category:p.category,journal:p.journal,doi:p.doi||p.source,method:p.method,date:p.date,access:p.access};return field==='all'?[p.title,p.authors,p.abstract,p.summary,p.tags.join(' '),p.material,p.method,p.journal,p.category,p.doi,p.source].join(' '):text[field]??''}
...
export function search(papers:Paper[],f:Filters,favorites:string[]=[]){
 ...
 const ranked=papers.filter(p=>(!node||matches(p,node,f.field))&&(!f.category||p.category===f.category)&&(!f.journal||p.journal===f.journal)&&(!f.method||p.method===f.method)&&(!f.date||p.date===f.date)&&(!f.access||accessLevel(p)===f.access)&&(!f.openAccess||(f.openAccess==='yes'?p.openAccess===true:f.openAccess==='no'?p.openAccess===false:p.openAccess===null))&&(!selectedTags.length||selectedTags.every(t=>p.tags.includes(t)||p.material===t))&&(!f.type||p.type===f.type)&&(!f.from||(p.year!==null&&p.year>=+f.from))&&(!f.to||(p.year!==null&&p.year<=+f.to))&&(!f.favorites||favorites.includes(p.id))).map(p=>({paper:p,score:terms.reduce((s,t)=>s+(normalize(p.title).includes(t)?12:0)+(normalize(p.tags.join(' ')).includes(t)?8:0)+(normalize(p.authors).includes(t)?6:0)+(normalize(p.abstract).includes(t)?4:0)+(normalize(p.summary).includes(t)?1:0),0)}));
