export type Paper = {
 ...
 growthRecipe?:import('./research-features.mjs').GrowthRecipe;
 evidenceSegments?:import('./research-features.mjs').EvidenceSegment[];
 evidenceProfile?:{fullText:string;sampleCredibility:string;reproducibility:string;physicalEvidence:string;certainty:string};
};
...
export const fields:Record<string,string>={all:'全部字段（含简报）',title:'标题',authors:'作者',institutions:'单位',abstract:'原始摘要',keywords:'关键词',journal:'期刊',doi:'DOI',summary:'简报分析',evidence:'证据段落',transportAgent:'输运剂',sourceTemperature:'源区温度',coolingRate:'降温速率'};
const aliases:Record<string,string>={title:'title',标题:'title',author:'authors',authors:'authors',作者:'authors',institution:'institutions',institutions:'institutions',单位:'institutions',机构:'institutions',abstract:'abstract',摘要:'abstract',keyword:'keywords',keywords:'keywords',关键词:'keywords',summary:'summary',简报:'summary',主题:'category',板块:'category',category:'category',期刊:'journal',journal:'journal',doi:'doi',方法:'method',method:'method',日期:'date',date:'date',全文:'access',access:'access',transport_agent:'transportAgent',输运剂:'transportAgent',source_temp:'sourceTemperature',源区温度:'sourceTemperature',cooling_rate:'coolingRate',降温速率:'coolingRate',evidence:'evidence',证据:'evidence'};
...
function values(p:Paper,field:string){const recipe=p.growthRecipe;const evidence=p.evidenceSegments?.map(s=>`${s.title} ${s.text}`).join(' ')||p.summary;const text:Record<string,string>={title:p.title,authors:p.authors,institutions:p.institutions||'',abstract:p.abstract,summary:[p.conclusion,p.summary].filter(Boolean).join(' '),keywords:p.tags.join(' '),category:p.category,journal:p.journal,doi:p.doi||p.source,method:[p.method,recipe?.method].filter(Boolean).join(' '),date:p.date,access:p.access,transportAgent:recipe?.transportAgent||'',sourceTemperature:recipe?.sourceTemperature||'',coolingRate:recipe?.coolingRate||'',evidence};return field==='all'?[p.title,p.authors,p.institutions,p.abstract,p.conclusion,p.summary,p.tags.join(' '),p.material,p.method,p.journal,p.category,p.doi,p.source,recipe?JSON.stringify(recipe):'',evidence].join(' '):text[field]??''}
...
export function search(papers:Paper[],f:Filters,favorites:string[]=[]){
 ...
 const selectedTags=f.tags.split('|').filter(Boolean);
 const ranked=papers.filter(p=>(!node||matches(p,node,f.field))&&(!f.category||p.category===f.category)&&(!f.journal||p.journal===f.journal)&&(!f.method||p.method===f.method)&&(!f.date||p.date===f.date)&&(!f.access||accessLevel(p)===f.access)&&(!f.openAccess||(f.openAccess==='yes'?p.openAccess===true:f.openAccess==='no'?p.openAccess===false:p.openAccess===null))&&(!selectedTags.length||selectedTags.every(t=>p.tags.includes(t)||p.material===t))&&(!f.type||p.type===f.type)&&(!f.from||(p.year!==null&&p.year>=+f.from))&&(!f.to||(p.year!==null&&p.year<=+f.to))&&(!f.favorites||favorites.includes(p.id))).map(p=>({paper:p,score:terms.reduce((s,t)=>s+(normalize(p.title).includes(t)?12:0)+(normalize(p.tags.join(' ')).includes(t)?8:0)+(normalize(p.authors).includes(t)?6:0)+(normalize(p.abstract).includes(t)?4:0)+(normalize(p.conclusion).includes(t)?3:0)+(normalize(p.summary).includes(t)?1:0),0)}));
...
export function citation(p:Paper,format:'bibtex'|'ris'|'apa'){
 const doi=p.doi||p.source.replace(/^https?:\/\/(?:dx\.)?doi.org\//i,'');const hasDoi=/^10\.\d{4,9}\//.test(doi);const authors=p.authors.split(/;|；/).map(s=>s.trim()).filter(s=>s&&!/详见|未报告|未知/.test(s));
 const clean=(v:string)=>v.replace(/[\r\n]+/g,' ').trim();
 if(format==='apa'){const authorText=authors.length?authors.join(', '):'作者未收录';const year=p.year?String(p.year):'n.d.';const link=hasDoi?'https://doi.org/'+doi:safeUrl(p.source);return `${authorText}. (${year}). ${clean(p.title)}. ${clean(p.journal)}. ${link}\n`;}
...
export function similarPapers(paper:Paper,papers:Paper[],limit=3){return papers.filter(p=>p.id!==paper.id).map(p=>({p,score:(p.category===paper.category?4:0)+(p.journal===paper.journal?2:0)+p.tags.filter(t=>paper.tags.includes(t)).length*3+(p.material===paper.material?3:0)})).filter(x=>x.score>0).sort((a,b)=>b.score-a.score||(b.p.published||'').localeCompare(a.p.published||'')).slice(0,limit).map(x=>x.p)}
